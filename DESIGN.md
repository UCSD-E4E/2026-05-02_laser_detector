...
- ~~Sub-pixel localization~~ (was a non-goal; implemented and landed +4.24 pp on test in Phase 2A — see §11 bf16 finding and `notes/phase2_results.md`).
...
- ~~Sub-pixel argmax (parabolic peak refinement) at inference — cheap, may close 1–2 px of the gap.~~ **Done (Phase 2A v3)**. Lifted val/test hit_n3 by +4.5 / +4.2 pp. The actual gain was from a coupled fix: refine on logits AND compute sigmoid in fp32. See §11 bf16 finding.
...
- **bf16 sigmoid argmax tie-break (root cause of the ~(−1.13, −2.07) px pixel-bias calibration, 2026-06-06)**: production inference ran under `torch.autocast(dtype=bfloat16)`. Inside autocast, `torch.sigmoid(heatmap_logits)` is computed in bf16 — and bf16 has ~8-bit mantissa precision near 1.0, so `sigmoid(x) > 1 − 2⁻⁸` for any `x > 5.5` saturates exactly to 1.0. At a confident laser peak (logits ≥ +6), multiple adjacent pixels saturate; the heatmap's bf16 representation makes them numerically identical. `tensor.max()` resolves ties to the lowest flat (row-major) index, biasing predictions toward the upper-left of the saturated cluster. Predictions land ~1 px left and ~2 px up of the true peak — within rounding of the (−1.13, −2.07) offset the hand-calibration was correcting. The earlier Bayer-excess-upsample and decoder-mode hypotheses (preserved in `notes/bias_attribution.md` for the historical record) were measuring this same effect through different lenses. **Fix**: promote logits to fp32 before sigmoid (`heatmap_logits = out["heatmap_logits"].float(); heatmap_probs = torch.sigmoid(heatmap_logits)`); refine the parabolic sub-pixel peak on logits, which are bf16-stable. Implemented in `src/laser_detector/inference.py` (`predict_frame` and `predict_frame_with_cascade`), exposed via `--subpixel-refine`. Empirical impact on production: val hit_n3 0.8498 → 0.8951, test 0.8120 → 0.8544. Calibration offset drops from (−1.13, −2.07) to (−0.20, −0.006), val-derived offset generalizes cleanly to test (0.05 pp gap). The run5_bilinear and run6_bayer_diff retrains were motivated by the misattributed cause; under v3 inference, run3 (nearest decoder, no G_diff) beats both. **Lesson**: when an "architectural" bias is uniform across rigs, wavelengths, and dives, suspect the inference pipeline before the model. _DESIGN doc gap: the sensor-coord + 6-channel + Bayer-excess pipeline predates this doc revision; §4.1's "4-channel input" is no longer the production configuration. A sync of §3–4 to reflect the sensor-coord refactor is still pending._
